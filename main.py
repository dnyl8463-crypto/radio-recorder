...
# הגדרות - ה-ID שלך והכתובות החדשות ששלחת
...
def get_drive_service():
    ...
    if not creds_json:
        raise Exception("Missing GOOGLE_CREDENTIALS secret in GitHub Settings")
...
def main():
    ...
    # פורמט שם קובץ: יום-חודש-שנה_שעה
    ...
    for name, url in STATIONS.items():
        try:
            ...
            print(f"מתחיל הקלטה של {name}...")
            
            # הקלטה של 60 דקות (3600 שניות)
            # אם תרצה בדיקה קצרה של דקה אחת, שנה את 3600 ל-60
            subprocess.run(['ffmpeg', '-i', url, '-t', '3600', '-acodec', 'libmp3lame', '-ab', '64k', filename], check=True)
            
            print(f"מעלה את {filename} לדרייב...")
            file_metadata = {'name': filename, 'parents': [folder_id]}
            media = MediaFileUpload(filename, mimetype='audio/mpeg')
            service.files().create(body=file_metadata, media_body=media).execute()
            
            os.remove(filename) # מנקה מקום בשרת
